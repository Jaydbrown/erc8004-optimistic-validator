...
export function StatusPanel({ requestHash }: Props) {
  ...
  const expiresIn = assertion && now !== null ? Number(assertion.expirationTime) - now : undefined;
  ...
  return (
    <section className="border border-neutral-800 rounded-md p-4 flex flex-col gap-4 bg-neutral-950/40">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
        ...
        <div className="flex flex-col gap-1">
          <span className="text-xs font-mono uppercase text-neutral-600">Validator Request</span>
          <span>Status: <b>{request ? REQUEST_STATUS[request.status] : "—"}</b></span>
          <span className="font-mono text-xs text-neutral-500 break-all">
        ...
        <div className="flex flex-col gap-1">
          <span className="text-xs font-mono uppercase text-neutral-600">Bonded Assertion</span>
          <span>Status: <b>{assertion ? ASSERTION_STATUS[assertion.status] : "—"}</b></span>
          <span>
